...
function Level:loadLevel(number)
  print(filename)
  ...
  for b=9, 1205, 4 do
    ...
    if output then
      ...
      table.insert(self.terrain[x], output)
      y = y + 1
    end
  end

  --The bytes 1209 until 2405 contain which parts of the level can't be moved over
  x, y = 1, 1
  for b=1209, 2405, 4 do
    output = string.byte(file, b)
    if output then
      ...
      if output > 0 then
        table.insert(self.collisionMap[x], true)
      else
        table.insert(self.collisionMap[x], false)
      end
      y = y + 1
    end
  end

  --The bytes 2409 until 3605 contain all the object 
  x, y = 1, 1
  for b=2409, 3605, 4 do
    output = string.byte(file, b)
    if output then
      ...
      table.insert(self.objects[x], output)
      y = y + 1
    end
  end
  ...
  for b=3609, 6005, 4 do
    ...
    table.insert(self.ground[x], output)
    y = y + 1
  end
